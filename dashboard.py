...
# ==========================================
# PAGE CONFIGURATION & THREADING FIX
# ==========================================
...
# STATE 9: Local Archiving Configuration
st.sidebar.divider()
...
if page == "Audit New Contracts":
    ...
    if uploaded_files:
        if st.button("🚀 Execute Batch Process"):
            ...
            # Ensure the archive directory exists before saving
            ...
            with st.spinner(f"Processing {len(uploaded_files)} documents through the agent pipelines..."):
                for uploaded_file in uploaded_files:
                    ...
                    save_path = os.path.join(archive_path, filename)
    ...
    if st.session_state.batch_results:
        ...
        for filename, details in list(st.session_state.batch_results.items()):
            ...
            with st.expander(title_string, expanded=(status == "Flagged for Review")):
                # UI CONFIRMATION OF ARCHIVE PATH
                st.caption(f"Archived securely at: `{details.get('saved_path', 'Not saved')}`")
